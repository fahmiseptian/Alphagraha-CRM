@php
    $brand = $brand ?? new \App\Models\Brand(['is_active' => true]);
@endphp
<div class="space-y-4">
    <div>
        <label class="crm-label">Nama Brand <span class="text-red-500">*</span></label>
        <input type="text" name="name" value="{{ old('name', $brand->name) }}" required maxlength="255"
               class="crm-field @error('name') border-red-400 @enderror" placeholder="Contoh: Acer">
    ...
    <div class="flex items-center gap-2 pt-2">
        ...
        <a href="{{ route('brands.index') }}" class="rounded-lg border border-slate-300 px-5 py-2 text-sm font-medium text-slate-600 hover:bg-slate-50">Cancel</a>
    </div>
</div>
